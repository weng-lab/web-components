...
} as const satisfies GridInitialStatePremium; 

const groupingColDef: GridGroupingColDefOverride<any> = {
  leafField: "displayname",
  headerName: "Tissue/Biosample",
  maxWidth: 400,
  display: "flex",
...
const addAutogeneratedRowIdsToChildren = (
  ...
  if (!apiRef.current) return new Set(selected)

  const entries = gridFilteredSortedRowEntriesSelector(apiRef)
  const autogeneratedRows = entries.filter((entry) =>
...
const getAndSetRowSelectionModel = (
  apiRef: React.RefObject<GridApiPremium | null>,
  selected: GridRowId[] | undefined,
  setRowSelectionModel: React.Dispatch<React.SetStateAction<GridRowSelectionModel | undefined>>
) => {
  if (!selected || !apiRef.current) return undefined;
  const newModel = { type: "include", ids: addAutogeneratedRowIdsToChildren(apiRef, selected) } as const;
...
export function BiosampleTable(props: BiosampleTableProps): JSX.Element {
  ...
  const {
    ...
    onDownload, //custom added prop for tracking downloads
  ...
  const handleRowSelectionModelChange: DataGridPremiumProps["onRowSelectionModelChange"] = (
    rowSelectionModel,
    details
  ) => {
  ...
  const [rowSelectionModel, setRowSelectionModel] = useState<GridRowSelectionModel | undefined>(undefined);

  useEffect(() => {
    if (!selected) return undefined;
    const newSelectionModel = { type: "include", ids: addAutogeneratedRowIdsToChildren(apiRef, selected) } as const;
    if (
      !rowSelectionModel ||
      !(
        rowSelectionModel.ids.isSubsetOf(newSelectionModel.ids) &&
        newSelectionModel.ids.isSubsetOf(rowSelectionModel.ids)
      )
    ) {
  ...
  const handleOnReady: typeof onReady = (apiRef) => {
    const internalCleanup = apiRef.current.subscribeEvent("filterModelChange", () => {
      getAndSetRowSelectionModel(apiRef, selectedRef.current, setRowSelectionModel);
    });
    const consumerCleanups = onReady?.(apiRef);
    if (!consumerCleanups) return internalCleanup;
    if (Array.isArray(consumerCleanups)) return [internalCleanup, ...consumerCleanups];
    return [internalCleanup, consumerCleanups];
  };

  return (
    <DownloadContext.Provider value={{ onDownload }}>
      <Table
        apiRef={apiRef}
        label={label}
        rows={internalRows}
