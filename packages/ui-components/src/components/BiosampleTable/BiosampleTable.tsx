...
import { DataGridPremiumProps, getGroupRowIdFromPath, GridApiPremium, GridColDef, gridDataRowIdsSelector, gridFilteredSortedRowEntriesSelector, GridGroupingColDefOverride, GridRowId, gridRowNodeSelector, GridRowSelectionModel, gridRowsLookupSelector, gridRowsMetaSelector, gridSortedRowEntriesSelector, gridVisibleRowsSelector, isAutogeneratedRow, useGridApiRef } from "@mui/x-data-grid-premium";
...
export function BiosampleTable(props: BiosampleTableProps): JSX.Element {
  ...
  const handleRowSelectionModelChange: DataGridPremiumProps["onRowSelectionModelChange"] = (rowSelectionModel, details) => {
    ...
    console.log("calling onSelectionChange with", internalRows.filter((row) => rowSelectionModel.ids.has(getRowId(row))))
  ...
  useEffect(() => {
    ...
    if (!rowSelectionModel || !(rowSelectionModel.ids.isSubsetOf(newSelectionModel.ids) && newSelectionModel.ids.isSubsetOf(rowSelectionModel.ids))) { //prevent infinite rerender
      setRowSelectionModel(newSelectionModel)
    }
  }, [selected]);

  useEffect(() => {
    return apiRef.current?.subscribeEvent("filterModelChange", () => {console.log("calling from other effect"); getAndSetRowSelectionModel(apiRef, selected, setRowSelectionModel)});
  }, [apiRef, selected]);
