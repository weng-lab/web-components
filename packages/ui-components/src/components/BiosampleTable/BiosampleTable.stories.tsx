...
import { DataGridPremiumProps, isAutogeneratedRow, useGridApiRef } from "@mui/x-data-grid-premium";
...
export const ByCellAndTissueDownloads: Story = {
  render: () => {
    ...
    const rowsHaveBeenExpanded = React.useRef(false)

    React.useEffect(() => {
      return apiRef.current?.subscribeEvent("rowExpansionChange", (params, event, details) => {
        if (!rowsHaveBeenExpanded.current) {
          ...
          rowsHaveBeenExpanded.current = true;
        }
      });
    }, [apiRef]);
