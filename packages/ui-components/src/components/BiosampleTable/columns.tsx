import { GridColDef, GridGroupingColDefOverride, gridRowIdSelector, gridRowNodeSelector, gridRowsLookupSelector, GridValidRowModel, isAutogeneratedRow } from "@mui/x-data-grid-premium";
  ...
  valueGetter: (_, row, __, apiRef) => {
    if (isAutogeneratedRow(row)) {
      ...
      if (isGroupingByOntology && typeof rowNode.groupingKey === "string"){
        return getAvailableFiles(rowNode.groupingKey).map(x => `https://downloads.wenglab.org/${x.filename}`).join(',')
      } else return null
    }
    const signalIDs = [
